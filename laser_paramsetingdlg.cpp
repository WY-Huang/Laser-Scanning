...
laser_paramsetingdlg::laser_paramsetingdlg(My_params *mcs, QWidget *parent):
    QDialog(parent),
    ui(new Ui::laser_paramsetingdlg)
{
    m_mcs = mcs;
...
laser_paramsetingdlg::~laser_paramsetingdlg()
{
  delete ui;
}

// 初始化读取相机参数
void laser_paramsetingdlg::Initparam(My_params *mcs)
{
    m_mcs = mcs;
    if(m_mcs->resultdata.link_param_state==true)
    {
...
void laser_paramsetingdlg::on_tabWidget_currentChanged(int index)
{
    switch(index)
    {
        case 0:
